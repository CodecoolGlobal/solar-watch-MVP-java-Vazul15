...
public class OpenWeatherService {
    ...
    public SunriseSunsetResponse getDemandedWeatherReport(CityReport city, LocalDate date) {
        ...
        Optional<SunriseSunset> existingSunriseSunsetData = getSunriseSunsetByDateAndCity(targetDate, city.name());

        if (existingSunriseSunsetData.isPresent()) {
            SunriseSunset sunriseSunset = existingSunriseSunsetData.get();
            return new SunriseSunsetResponse(
                    new SunriseSunsetResults(sunriseSunset.getSunrise(), sunriseSunset.getSunset())
            );
        }

        String formattedDate = buildString(targetDate);
        String url = String.format("https://api.sunrise-sunset.org/json?lat=%s&lng=%s&date=%s&formatted=0",
                city.lat(), city.lon(), formattedDate);
        ...
        if (response != null) {
            ...
            newSunriseSunset.setCity(cityRepository.findByName(city.name())
                    .orElseThrow(() -> new RuntimeException("City not found")));
            newSunriseSunset.setDate(targetDate);
